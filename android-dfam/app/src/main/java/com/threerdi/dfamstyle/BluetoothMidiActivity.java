...
public final class BluetoothMidiActivity extends Activity implements BleMidiSyncManager.Listener {
    ...
    private void buildUi() {
        ...
        // LEFT: independently scrollable so controls can never be clipped on
        // short/wide phones or when Android uses a large display scale.
        ScrollView leftScroll = new ScrollView(this);
        leftScroll.setFillViewport(true);
        leftScroll.setVerticalScrollBarEnabled(true);
        LinearLayout left = new LinearLayout(this);
        left.setOrientation(LinearLayout.VERTICAL);
        left.setPadding(dp(8), dp(4), dp(10), dp(8));
        leftScroll.addView(left, new ScrollView.LayoutParams(
                ScrollView.LayoutParams.MATCH_PARENT,
                ScrollView.LayoutParams.WRAP_CONTENT));
        root.addView(leftScroll, new LinearLayout.LayoutParams(
                0, LinearLayout.LayoutParams.MATCH_PARENT, 1.05f));

        TextView title = label("BLUETOOTH MIDI SYNC", 21, true);
        title.setPadding(0, 0, 0, dp(2));
        left.addView(title);

        TextView subtitle = label(
                "Standard BLE-MIDI • MIDI Clock 24 PPQN • Start / Continue / Stop",
                10.5f, false);
        subtitle.setPadding(0, 0, 0, dp(7));
        left.addView(subtitle);

        statusView = label("", 12, true);
        statusView.setBackgroundColor(panelLight);
        statusView.setPadding(dp(9), dp(7), dp(9), dp(7));
        left.addView(statusView, new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT));

        syncView = label("", 14, true);
        syncView.setPadding(0, dp(7), 0, dp(5));
        ...
        master.setOnClickListener(v -> {
            ...
            if (!midi.canAdvertise()) {
                Toast.makeText(this,
                        "This phone cannot advertise as a BLE MIDI peripheral",
        ...
        TextView note = label(
                ...
                9.5f, false);
        note.setPadding(0, dp(7), 0, dp(10));
        left.addView(note);

        // RIGHT: title + independently scrolling device list + fixed Back button.
        LinearLayout right = new LinearLayout(this);
        right.setOrientation(LinearLayout.VERTICAL);
        right.setPadding(dp(10), dp(4), dp(8), dp(8));
        root.addView(right, new LinearLayout.LayoutParams(
                0, LinearLayout.LayoutParams.MATCH_PARENT, 0.95f));

        TextView devicesTitle = label("BLE MIDI DEVICES", 16, true);
        devicesTitle.setPadding(0, 0, 0, dp(3));
        right.addView(devicesTitle);

        ScrollView deviceScroll = new ScrollView(this);
        deviceScroll.setFillViewport(true);
        deviceScroll.setVerticalScrollBarEnabled(true);
        deviceList = new LinearLayout(this);
        deviceList.setOrientation(LinearLayout.VERTICAL);
        deviceList.setPadding(0, dp(4), 0, dp(4));
        deviceScroll.addView(deviceList, new ScrollView.LayoutParams(
                ScrollView.LayoutParams.MATCH_PARENT,
                ScrollView.LayoutParams.WRAP_CONTENT));
        right.addView(deviceScroll, new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, 0, 1f));

        Button back = button("BACK TO SYNTH");
        back.setOnClickListener(v -> finish());
        LinearLayout.LayoutParams backParams = buttonParams();
        backParams.topMargin = dp(5);
        right.addView(back, backParams);
    ...
    private int contrastText(int c) {
        double luminance = 0.299 * Color.red(c)
                + 0.587 * Color.green(c)
                + 0.114 * Color.blue(c);
    ...
    public void onRequestPermissionsResult(
        ...
        if (requestCode == REQ_BT) {
            ...
            Toast.makeText(this,
                    granted
                            ? "Bluetooth MIDI permission granted"
                            : "Bluetooth MIDI needs Bluetooth permission",
                    Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void onSyncChanged(float bpm, boolean running) {
        runOnUiThread(() -> {
            ...
            if (midi.isMasterActive()) {
                ...
                if (bpm > 0f) {
                    syncView.setText(String.format(Locale.US,
                            "SLAVE SYNC • %.2f BPM • %s",
                            bpm, running ? "RUNNING" : "STOPPED"));
                } else {
                    syncView.setText("SLAVE SYNC • waiting for MIDI Clock");
                }
            } else {
    ...
    private void refreshDevices() {
        ...
        if (devices.isEmpty()) {
            TextView empty = label(
                    "No BLE MIDI devices found yet.\nPress SLAVE • SCAN / RECEIVE CLOCK.",
                    11, false);
